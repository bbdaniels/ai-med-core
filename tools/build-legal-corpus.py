...
def api_post(env: dict[str, str], path: str, payload: dict, retries: int = 4) -> dict:
    ...
    for attempt in range(retries):
        try:
            with urllib.request.urlopen(req, timeout=180) as resp:
                data = resp.read()
                # The Harvard HUIT gateway gzips whatever the request asked for,
                # and urllib -- unlike requests -- does no content decoding.
...
def embed_batch(env: dict[str, str], texts: list[str]) -> list[list[float]]:
    data = api_post(env, "/embeddings", {"model": EMBED_MODEL, "input": texts})
